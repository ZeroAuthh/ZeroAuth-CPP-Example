...
namespace settings {
    namespace input {
        ImVec4 text_selected = ImColor(10, 10, 10, 50);
        ImVec4 text_hint = ImColor(180, 180, 180, 255);
        ImVec4 text = ImColor(255, 255, 255, 255);
        ImVec4 background_hov = ImColor(30, 30, 30, 100);
        ImVec4 background = ImColor(70, 70, 70, 60);
    };
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        ImVec4 background;
        ImVec4 rect_color, text_color;
        float value_x, speed_x;
    };
    ...
    if (!is_multiline)
    {
        RenderNavCursor(frame_bb, id);
        // Sem interpolação
        it_anim->second.background = ImLerp(it_anim->second.background, settings::input::background_hov, g.IO.DeltaTime * 6.f);
        // Com interpolação
        //it_anim->second.background = ImLerp(it_anim->second.background,(IsItemActive() || hovered) ? settings::input::background_hov : settings::input::background,g.IO.DeltaTime * 6.f);
        ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(it_anim->second.background), true, style.FrameRounding);
    }
    ...
    if (is_multiline) {
        ...
        draw_pos = frame_bb.Min + ImVec2(5.0f, frame_bb.GetHeight() * 0.5f - label_size.y * 0.5f);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 text_selected_u32 = ImGui::GetColorU32(settings::input::text_selected);
            ImU32 bg_color = ImGui::GetColorU32(text_selected_u32, render_cursor ? 1.0f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, style.FrameRounding);
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? settings::input::text_hint : settings::input::text);
        ...
        if (render_cursor)
        {
            ...
            if ((frame_bb.Min.x + it_anim->second.value_x) < cursor_screen_rect.Min.x) {
                it_anim->second.speed_x = (cursor_screen_rect.Min.x - (frame_bb.Min.x + it_anim->second.value_x)) / 10.f;
                it_anim->second.value_x += it_anim->second.speed_x / ImGui::GetIO().Framerate * 200.f;
            }
            else if ((frame_bb.Min.x + it_anim->second.value_x) > cursor_screen_rect.Min.x) {
                it_anim->second.speed_x = ((frame_bb.Min.x + it_anim->second.value_x - cursor_screen_rect.Min.x)) / 10.f;
                it_anim->second.value_x -= it_anim->second.speed_x / ImGui::GetIO().Framerate * 200.f;
            }

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ImVec2 animated_cursor_pos = cursor_screen_rect.Min;
                animated_cursor_pos.x = frame_bb.Min.x + it_anim->second.value_x;
                draw_window->DrawList->AddLine(animated_cursor_pos, ImVec2(animated_cursor_pos.x, cursor_screen_rect.Max.y), ImGui::GetColorU32(ImGuiCol_Text));
            }
