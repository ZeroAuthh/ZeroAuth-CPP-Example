...
bool ImGui::ButtonAnimadoo12(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map<ImGuiID, button12Anims> anim; // #include <map> on top of this file, where is "imgui.h" located.
    ...
    it_anim->second.closing_anim = ImLerp(it_anim->second.closing_anim, (hovered || held ? size.x : 0), g.IO.DeltaTime * 8.f);
    ...
    const ImU32 outside_solid_col = ImColor(40, 40, 40, 90);
    const ImU32 inside_hover_col = ImColor(125, 0, 0, (int)it_anim->second.closing_alpha);

    draw->AddRectFilled(bb.Min, ImVec2(bb.Min.x + it_anim->second.closing_anim, bb.Max.y), inside_hover_col, 5);
    draw->AddRect(bb.Min, bb.Max, outside_solid_col, 5);

    PushStyleColor(ImGuiCol_Text, ColorConvertFloat4ToU32(ImColor(125, 0, 0, 255 - (int)it_anim->second.label_alpha)));
    if (g.LogEnabled)
...
bool ImGui::ButtonAnimadoo9(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map<ImGuiID, button9Anims> anim; // #include <map> on top of this file, where is "imgui.h" located.
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
    ...
    it_anim->second.moveYaxis = ImLerp(it_anim->second.moveYaxis, (hovered || held ? 4.f : 0), g.IO.DeltaTime * 6.f);
    it_anim->second.moveXaxis = ImLerp(it_anim->second.moveXaxis, (hovered || held ? 4.f : 0), g.IO.DeltaTime * 6.f);

    if (hovered || held) {
        if (it_anim->second.alpha < 255.f)
            it_anim->second.alpha += 5.f / GetIO().Framerate * 160.f;
    }
    else {
        if (it_anim->second.alpha > 0.f)
            it_anim->second.alpha -= 5.f / GetIO().Framerate * 160.f;
    }


    // Render
    const ImU32 inside_solid_col = ImColor(220, 20, 60, 255);
    const ImU32 inside_hover_col = ImColor(101, 0, 255, (int)it_anim->second.alpha);
    draw->AddRectFilled(bb.Min + ImVec2(4, 4) - ImVec2(it_anim->second.moveXaxis, it_anim->second.moveYaxis), bb.Max + ImVec2(4, 4) - ImVec2(it_anim->second.moveXaxis * 3.0f, it_anim->second.moveYaxis * 3.0f), IM_COL32_BLACK, 4);
    draw->AddRectFilled(bb.Min + ImVec2(it_anim->second.moveXaxis, it_anim->second.moveYaxis), bb.Max - ImVec2(it_anim->second.moveXaxis, it_anim->second.moveYaxis), inside_solid_col, 4);
    draw->AddRectFilled(bb.Min + ImVec2(it_anim->second.moveXaxis, it_anim->second.moveYaxis), bb.Max - ImVec2(it_anim->second.moveXaxis, it_anim->second.moveYaxis), inside_hover_col, 4);
...
bool ImGui::ButtonAnimadoo14(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map<ImGuiID, button14Anims> anim; // #include <map> on top of this file, where is "imgui.h" located.
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
    ...
    draw->AddRectFilled(ImVec2((bb.Min.x + size.x / 2.f) - it_anim->second.closing_anim_X, (bb.Min.y + size.y / 2.f) - it_anim->second.closing_anim_Y), ImVec2((bb.Max.x - size.x / 2.f) + it_anim->second.closing_anim_X, (bb.Max.y - size.y / 2.f) + it_anim->second.closing_anim_Y), inside_hover_col, 4);
    draw->AddRect(bb.Min, bb.Max, outside_solid_col, 4, 0, 2);
...
bool ImGui::CloutButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    // name == label
    ...
    //ImVec4 circle_color = ImVec4(0.98f, 0.98f, 0.98f, it_circle->second.mult * 0.1f);
    ImVec4 circle_color = ImVec4(1.0f, 0.0f, 0.0f, it_circle->second.mult * 0.1f);
    ...
    //ImColor hovering_color = ImColor(0.57f, 0.38f, 0.91f, it_hover->second * 0.4f);

    ImColor hovering_color = ImColor(1.0f, 0.0f, 0.0f, it_hover->second * 1.0f);


    window->DrawList->AddRect(bb.Min, bb.Max, hovering_color, GetStyle().FrameRounding);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, GetStyle().FrameRounding);
...
bool ImGui::ButtonClout(const char* label, const ImVec2& size_arg)
{
    return CloutButton(label, size_arg, ImGuiButtonFlags_None);
}


bool ImGui::ButtonAnimado14(const char* label, const ImVec2& size_arg)
{
    return ButtonAnimadoo14(label, size_arg, ImGuiButtonFlags_None);
}

bool ImGui::ButtonAnimado9(const char* label, const ImVec2& size_arg)
{
    return ButtonAnimadoo9(label, size_arg, ImGuiButtonFlags_None);
}

bool ImGui::ButtonAnimado12(const char* label, const ImVec2& size_arg)
{
    return ButtonAnimadoo12(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    const ImVec2 center = pos + ImVec2(buttonSize * 0.5f, buttonSize * 0.f);
    const ImRect bb(pos, pos + ImVec2(buttonSize, buttonSize));
    ...
    const ImU32 bgColor = GetColorU32(held ? ImGuiCol_CloseButtonActive : hovered ? ImGuiCol_CloseButtonActive : ImGuiCol_ButtonHovered);
...
bool ImGui::MinimizeButton(ImGuiID id, const ImVec2& pos)
{
    ...
    window->DrawList->AddLine(center + ImVec2(cross_extent + 10, cross_extent - 1), center + ImVec2(cross_extent, cross_extent - 1), cross_col, 1.0f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.2f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        ...
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), false, style.FrameRounding);
        //draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(255, 255, 255, 15), 5);
        ...
        //draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(255, 0, 0, it_anim->second.active_rect_alpha), 5);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(-2, 0), rect.Max + ImVec2(3, 3), ImColor(25, 25,25), 0);
                }
        ...
        if (render_cursor)
        {
            ...
            if ((frame_bb.Min.x + it_filing->second.value) < cursor_screen_rect.Min.x) {
                it_filing->second.speed = (cursor_screen_rect.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 7.f;
                ...
                it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - cursor_screen_rect.Min.x)) / 7.f;
                it_filing->second.value -= it_filing->second.speed / ImGui::GetIO().Framerate * 200.f;
            }

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))

                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_filing->second.value + 0, 6), frame_bb.Min + ImVec2(it_filing->second.value + 1, 30), GetColorU32(ImGuiCol_Text));
    ...
    if (label_size.x > 0) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - CalcItemWidth() - 171, frame_bb.Min.y + style.FramePadding.y - 2), label);
